...
// ---------------------------------------------------------------------------
    ...
    // Initialize Interface State Class if needed and return StateSaveClass
    [SerializeField] private MAST_Interface_State_Save StateSaveClass;
    private MAST_Interface_State_Save StateSave
    {
        get
        {
            if(StateSaveClass == null)
                StateSaveClass = new MAST_Interface_State_Save();
            return StateSaveClass;
        }
    }
    ...
    [SerializeField] public static int paletteColumnCount = 3;
    ...
    private void OnEnable()
    {
        ...
        if (!inPlayMode)
        {
            ...
            StateSave.Load_Interface_State();
            
            // Create a new grid if needed
            if (StateSave.state.gridExists)
            {
    ...
    private void OnDisable()
    {
        ...
        if (!inPlayMode)
            StateSave.Save_Interface_State();
    ...
    private void DisplayToolbarGUI()
    {
            ...
            "MAST Button", GUILayout.Width(toolBarIconSize), GUILayout.Height(toolBarIconSize)))
        {
            ...
            if (EditorUtility.DisplayDialog("Are you sure?",
                ...
                "Load Prefabs", "Cancel"))
            {
                // Process prefabs for palette
                MAST_Palette.ProcessPrefabsInSelectedFolder();
                
                // Save state
                StateSave.Save_Palette_State(true);
            }
        }
...
// ---------------------------------------------------------------------------
    ...
    private void DisplayPaletteGUIPopulated()
    {
        ...
        int rowCount = Mathf.CeilToInt(MAST_Palette.GetGUIContentArray().Length / (float)paletteColumnCount);
